#! /usr/bin/python

import codecs

def xor(s1, s2):
	return tuple(a^b for a,b in zip(s1, s2))

def gmul(a, b):
	p = 0
	for c in range(8):
		if b & 1:
			p ^= a
		a <<= 1
		if a & 0x100:
			a ^= 0x11b
		b >>= 1
	return p

Gmul = {}
for f in (0x02, 0x03, 0x0e, 0x0b, 0x0d, 0x09):
	Gmul[f] = tuple(gmul(f, x) for x in range(0,0x100))

def show_block(block, info):
	print(info+':')
	for i in range(0, len(block), 16):
		output_string=''.join(format(e, '02x') for e in block[i:i+16]).replace('0x', '')
		print(str(i//16)+': \''+output_string+'\'')

class AES(object):
	def __init__(self, name, bases, classdict):
		print('AES Init...')

	Rcon = ( 0x8d, 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36, 0x6c, 0xd8, 0xab, 0x4d, 0x9a )
	Sbox = (
			
			)
	Sbox_inv = (
			
			)

	key_scheduled = False

	def rot_word(word):
		return word[1:] + word[:1]

	def sub_word(word):
		return (AES.Sbox[b] for b in word)

	def key_schedule(self):
		if self.key_scheduled:
			return self.key
		else:
			expanded = self.key
			for i in range(self.nk, self.nb * (self.nr + 1)):
				t = expanded[(i-1)*4:i*4]
				if i % self.nk == 0:
					t = xor( AES.sub_word( AES.rot_word(t) ), (AES.Rcon[i // self.nk],0,0,0) )
				elif self.nk > 6 and i % self.nk == 4:
					t = AES.sub_word(t)
				expanded.extend( xor(t, expanded[(i-self.nk)*4:(i-self.nk+1)*4]))
			self.key_scheduled = True
			return expanded

	def add_round_key(self, rkey):
		for i, b in enumerate(rkey):
			self.state[i] ^= b

	def sub_bytes(self):
		for i, b in enumerate(self.state):
			self.state[i] = AES.Sbox[b]

	def inv_sub_bytes(self):
		for i, b in enumerate(self.state):
			self.state[i] = AES.Sbox_inv[b]

	def shift_rows(self):
		rows = []
		for r in range(4):
			rows.append( self.state[r::4] )
			rows[r] = rows[r][r:] + rows[r][:r]
		self.state = [ r[c] for c in range(4) for r in rows ]

	def inv_shift_rows(self):
		rows = []
		for r in range(4):
			rows.append( self.state[r::4] )
			rows[r] = rows[r][4-r:] + rows[r][:4-r]
		self.state = [ r[c] for c in range(4) for r in rows ]

	def mix_columns(self):
		ss = []
		for c in range(4):
			col = self.state[c*4:(c+1)*4]
			ss.extend((
				Gmul[0x02][col[0]] ^ Gmul[0x03][col[1]] ^ col[2]  ^ col[3] ,
				col[0]  ^ Gmul[0x02][col[1]] ^ Gmul[0x03][col[2]] ^ col[3] ,
				col[0]  ^ col[1]  ^ Gmul[0x02][col[2]] ^ Gmul[0x03][col[3]],
				Gmul[0x03][col[0]] ^ col[1]  ^ col[2]  ^ Gmul[0x02][col[3]],
			))
		self.state = ss

	def inv_mix_columns(self):
		ss = []
		for c in range(4):
			col = self.state[c*4:(c+1)*4]
			ss.extend((
						Gmul[0x0e][col[0]] ^ Gmul[0x0b][col[1]] ^ Gmul[0x0d][col[2]] ^ Gmul[0x09][col[3]],
						Gmul[0x09][col[0]] ^ Gmul[0x0e][col[1]] ^ Gmul[0x0b][col[2]] ^ Gmul[0x0d][col[3]],
						Gmul[0x0d][col[0]] ^ Gmul[0x09][col[1]] ^ Gmul[0x0e][col[2]] ^ Gmul[0x0b][col[3]],
						Gmul[0x0b][col[0]] ^ Gmul[0x0d][col[1]] ^ Gmul[0x09][col[2]] ^ Gmul[0x0e][col[3]],
					))
		self.state = ss

	def cipher(self, block, cipher_cache):
		n = self.nb * 4
		self.state = block
		keys = self.key_schedule()
		show_block(keys, 'keys')
		self.add_round_key(keys[0:n])
		show_block(self.state, 'AddRoundKey')
		cipher_cache.extend(self.state)
		for r in range(1, self.nr):
			self.sub_bytes()
			if r==1:
				show_block(self.state, 'SubByte')
			self.shift_rows()
			if r==1:
				show_block(self.state, 'ShiftRows')
			self.mix_columns()
			if r==1:
				show_block(self.state, 'MixColumns')
			k = keys[r*n:(r+1)*n]
			self.add_round_key(k)
			if r==1:
				show_block(self.state, 'AddRoundKey')
			cipher_cache.extend(self.state)

		self.sub_bytes()
		self.shift_rows()
		self.add_round_key(keys[self.nr*n:(self.nr+1)*n])
		return self.state

	def inv_cipher(self, block):
		n = self.nb * 4
		self.state = block
		keys = self.key_schedule()
		k = keys[self.nr*n:(self.nr+1)*n]
		self.add_round_key(k)
		for r in range(self.nr-1, 0, -1):
			self.inv_shift_rows()
			self.inv_sub_bytes()
			k = keys[r*n:(r+1)*n]
			self.add_round_key(k)
			self.inv_mix_columns()

		self.inv_shift_rows()
		self.inv_sub_bytes()
		self.add_round_key(keys[0:n])
		return self.state



class AES_128(AES):
	def __init__(self):
		self.nb = 4
		self.nr = 4
		self.nk = 4
	
if __name__=="__main__":
	decode_hex=codecs.getdecoder('hex_codec')
	key = decode_hex('a0000000000000000000000000000000')[0]
	key = [e for e in key]
	##"f69f2445df4f9b17ad2b417be66c3710", "7b0c785e27e8ad3f8223207104725dd4"
	m1 = decode_hex('f0000000000000000000000000000000')[0]
	m1 = [e for e in m1]
	m2 = decode_hex('70000000000000000000000000000000')[0]
	m2 = [e for e in m2]
	diff=xor(m1,m2)
	crypt = AES_128()
	crypt.key = key
	m1_cache = []
	m2_cache = []
	t1 = crypt.cipher(m1, m1_cache)
	t2 = crypt.cipher(m2, m2_cache)
	diff = xor(m1_cache, m2_cache)
	i = 0
	for i in range(0, len(diff), 16):
		output_string=''.join(format(e, '02x') for e in diff[i:i+16]).replace('0x', '')
		print('diff'+str(i//16)+': \''+output_string+'\'')
