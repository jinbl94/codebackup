...
def string_to_bit_array(text,n):
	array = list()
	for char in text:
		#Get the char value on one byte
		binval = binvalue(char, n)
		#Add the bits to the final list
		array.extend([int(x) for x in list(binval)])
	return array

#Recreate the string from the bit array
def bit_array_to_string(array,n):
	res = ''.join([chr(int(y,2)) for y in [''.join([str(x) for x in bytes]) for bytes in nsplit(array,n)]])
	return res
...
class des():
	...
	def run(self, key, text, action=ENCRYPT, padding=False):
		...
		for block in text_blocks:#Loop over all the blocks of data
			block = string_to_bit_array(block,8)#Convert the block in bit array
		...
		final_res = bit_array_to_string(result,8)
	...
	def generatekeys(self):
		self.keys = []
		key = string_to_bit_array(self.password,8)
...
if __name__ == '__main__':
	key_hex='55555555555555'
	key_int=[ord(e)-ord('0') for e in key_hex]
	key_array=string_to_bit_array(key_int,4)
	text_hex='FEFEFEFEFEFEFEFE'
	key_full_array=[0]*64
	j=0
	for i in range(len(key_full_array)):
		if (i+1)%8==0:
			continue
		key_full_array[i]=key_array[j]
		j+=1
	key=bit_array_to_string(key_full_array,8)
	text=''.join(chr(ord(e)-ord('A')+10) for e in text_hex)
	text=string_to_bit_array(text,4)
	text=bit_array_to_string(text,8)
#key = "secret_k"
	d = des()
