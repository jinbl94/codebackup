...
bool Des::Des_Go(char *Out,char *In,long datalen,const char *Key,int keylen,bool Type){
	...
	//count
	//cout<<"\t";
	//for(int i=0;i<64;i++)cout<<i%10;cout<<endl;
	if( !Is3DES ) {   // 1次DES
...
void Des::DES(char Out[8],char In[8],const PSubKey pSubKey,bool Type){
	static bool M[64],tmp[32],*Li=&M[0],*Ri=&M[32],temp[64];
	int i=0;
	ByteToBit(M,In,64);
	//show the plaintext in binary model
	cout<<"M=";
	for(i=0;i<64;i++)cout<<M[i];cout<<endl;
...
//		M[k]=(M[k]+1)%2;
	if(Type==en){
		...
		cout<<"please enter leagle value for Type"<<endl;
		_exit(1);
	}
	Transform(M,M,IPR_Table,64);
	//show the cyphertex in binary model
	cout<<"C=";//<<k+1<<"\t\t";
	for(i=0;i<64;i++)cout<<M[i];cout<<endl;
//}
	BitToByte(Out,M,64);
...
void usage(void){
	fprintf(stdout,"USAGE: ./des m c Type(e/d)\n");
	_exit(1);
}
int main(int argc,char** argv){
	...
	if(!file_k){
		cout<<"failed to open data/key"<<endl;
		_exit(1);
	}
	...
	datalen=file_in.tellg();
	//adjust the length of plaintext
	...
	for(;i<datalen;i++)I[i]='\0';
	des.Des_Go(O,I,datalen,K,keylen,mode);
	for(i=0;i<datalen;i++)
		file_out<<O[i];
	delete [] K;
	delete [] I;
